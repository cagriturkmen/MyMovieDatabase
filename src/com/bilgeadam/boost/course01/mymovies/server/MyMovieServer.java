...
public class MyMovieServer {
	...
	private void start() {
		...
		// (1)  e�er gerekli ise TSV'lerden verileri okuyacak, parse edecek ve seriyalize edecek, yoksa do�rudan verileri deserialize edecek
	...
	private void startServer() {
		try (ServerSocket server = new ServerSocket(Props.getInstance().getServerPort());) {
			...
			while (true) {
				...
				ClientHandler clienthandler = new ClientHandler(client); 	// create a new thread object her yeni ba�lant� iste�i geldi�inde bir clientHandle olu�turuyor
				new Thread(clienthandler).start(); 							// This thread will handle the client separately ve onu yeni bir thread i�inde �al��t�r�yor
